...
import 'package:traveltest_app/profile_chat_section.dart'; // Import the chat section
import 'package:traveltest_app/settingsPage.dart';
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        // Load user stats if userId is available
        if (userId != null) {
          await _loadUserStats();
        }
  ...
  Future<void> _loadUserStats() async {
    try {
      postCount = await DatabaseMethods().getUserPostCount(userId!);
      followerCount = await DatabaseMethods().getFollowerCount(userId!);
      followingCount = await DatabaseMethods().getFollowingCount(userId!);
      setState(() {});
    } catch (e) {
  ...
  // FIXED: Complete _updateUserProfile method with email-based sync
    ...
    try {
      ...
      if (user != null) {
        ...
        if (image != null && image!.isNotEmpty) {
          try {
            await NetworkImage(image!).evict();
          } catch (e) {
        ...
        // 4. MAIN FIX: Update ALL user documents with same email
        if (userId != null && email != null && email!.isNotEmpty) {
          ...
          try {
            print('🔄 Starting profile update for email: $email');

            // Step 1: Find ALL documents with same email
            QuerySnapshot sameEmailUsers = await FirebaseFirestore.instance
                .collection('users')
                .where('Email', isEqualTo: email!.toLowerCase().trim())
                .get();

            print(
                '📧 Found ${sameEmailUsers.docs.length} users with email: $email');

            // Step 2: Update ALL documents with same email using batch
            WriteBatch batch = FirebaseFirestore.instance.batch();

            for (DocumentSnapshot doc in sameEmailUsers.docs) {
              batch.set(doc.reference, updateData, SetOptions(merge: true));
              print('📝 Adding to batch: ${doc.id}');
            }

            // Step 3: Also ensure current user document exists and is updated
            DocumentReference currentUserRef =
                FirebaseFirestore.instance.collection('users').doc(userId!);
            batch.set(currentUserRef, updateData, SetOptions(merge: true));
            print('📝 Adding current user to batch: $userId');

            // Execute batch update
            await batch.commit();
            print('✅ Batch update completed successfully');

            // Step 4: Verify the update after a small delay
            await Future.delayed(Duration(milliseconds: 800));
            QuerySnapshot verifyUsers = await FirebaseFirestore.instance
                .collection('users')
                .where('Email', isEqualTo: email!.toLowerCase().trim())
                .get();

            print(
                '🔍 Verification: Found ${verifyUsers.docs.length} updated users');
            for (DocumentSnapshot doc in verifyUsers.docs) {
              Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
              print('✅ Verified ${doc.id}: Image = ${data['Image']}');
            }
          } catch (e) {
            print('❌ Error updating Firestore: $e');

            // Fallback: Update using DatabaseMethods
            try {
              ...
              await DatabaseMethods().updateUserDetails(fallbackData, userId!);
              print('✅ Fallback: DatabaseMethods updated');
            } catch (dbError) {
              print('❌ DatabaseMethods also failed: $dbError');
            }
          }
        }
